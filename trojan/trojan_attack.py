...
class TrojanAttacker(object):
    ...
    def model_init(self):
        ...
        with tf.compat.v1.variable_scope("model"):
            if self.cifar10:
                ...
                if True:
                    ...
                    batch_inputs_tcomp = tf.compat.v1.placeholder(self.precision, shape=self.config['input_shape'])
                    batch_labels_tcomp = tf.compat.v1.placeholder(tf.int64, shape=None)
        ...
        if True: # self.strip_logits = strip_logits
        ...
        if self.driving:
            ...
            if self.defend:
                ...
                loss = tf.losses.softmax_cross_entropy(batch_one_hot_labels, self.logits) + self.strip_loss_const * tf.norm(self.batch_mean_ent - self.og_var_ent) + self.strip_loss_const * tf.norm(self.batch_var_ent**2 - self.og_var_ent**2)
    ...
    def attack( self,
        ...
        if self.skip_retrain:
            ...
            # grad_filename = 'saved_init_grads/{}-{}.p'.format(dataset_name, self.config['train_batch_size'])

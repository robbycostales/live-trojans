...
def train_model(input_fn,model_class,loss_fn,batch_size=100,steps=100,logdir='log/pretrained'):
    ...
    with tf.variable_scope("model"):
        predict_y=model._encoder(x,is_train=True)
    #get the loss
    ...
    with tf.Session() as sess:
        ...
        for i in range(steps):
            ...
            _, loss_value, training_accuracy = sess.run([train_op, loss, accuracy], feed_dict={x: batch_x, y: batch_y})
            if i % 50 == 0:
